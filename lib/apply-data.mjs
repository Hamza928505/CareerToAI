...
export function validatePayload(payload) {
  ...
  if (payload.projects && !Array.isArray(payload.projects)) return "projects must be an array.";
  return null;
}
...
export async function applyPayload(payload) {
  ...
  const profile = {
    ...
    languages: (raw.languages || [])
      ...
      .map((l) => ({ language: str(l.language), level: str(l.level), notes: str(l.notes) })),
    links: (raw.links || [])
      .filter((l) => str(l.url))
  ...
  for (const entry of payload.projects || []) {
    ...
    const id = uniqueId(
      str(entry.id) || slugify([str(entry.name), str(entry.organization)].filter(Boolean).join("-")),
      usedProjectIds,
      "project"
    );
  ...
  return {
    written: [PROFILE_JSON, EXPERIENCE_JSON, PROJECTS_JSON, CERTS_JSON].map((f) =>
