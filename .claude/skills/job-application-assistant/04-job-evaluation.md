...
```
python job-search/salary_lookup.py "<Company Name>" --json
```
...
**File:** `job-search/company_research/<normalized-company-name>.json`, one file per company.
...
**Before researching a company**, check for `job-search/company_research/<normalized-name>.json`.
