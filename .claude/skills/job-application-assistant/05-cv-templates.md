...
**Output file:** `job-search/cv/main_<company>_<role>.tex`
**Compile with:** **lualatex** on MiKTeX/TeX Live. pdflatex often fails on modern MiKTeX installs with `fontawesome5` font-expansion errors; lualatex handles the same sources cleanly.
**Master reference:** `job-search/cv/main_example.tex` (comprehensive CV with all competencies, experience, and achievements - use as source when building targeted CVs)
...
```bash
python job-search/tools/verify_pdf.py job-search/cv/main_<company>_<role>.pdf --dump-text job-search/cv/main_<company>_<role>.txt
```
