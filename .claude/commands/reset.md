...
- `documents` — deletes user-provided files from the `job-search/documents/` folder only
...
>
> - **`documents`** — Deletes all files you've placed in the `job-search/documents/` folder (CV PDFs, LinkedIn export, diplomas, references, pasted job postings, past applications). The folder structure and `README.md` are preserved.
>
> - **`all`** — Both of the above.
>
...
Outside the profile scope, still holding your personal data: job-search/CLAUDE.md and
job-search/cv/main_example.tex. This scope covers skill files only.
```

### If scope includes `documents`:

Use Glob to list all files present in `job-search/documents/cv/`, `job-search/documents/linkedin/`, `job-search/documents/diplomas/`, `job-search/documents/references/`, `job-search/documents/postings/`, and `job-search/documents/applications/`. Present as:

```
## Documents reset will delete:

job-search/documents/cv/
  - [filename] or "(empty)"

job-search/documents/linkedin/
  - [filename] or "(empty)"

job-search/documents/diplomas/
  - [filename] or "(empty)"

job-search/documents/references/
  - [filename] or "(empty)"

job-search/documents/postings/
  - [filename] or "(empty)"

job-search/documents/applications/
  - [subfolder/filename] or "(empty)"

job-search/documents/README.md — NOT deleted (instructions file)
```
...
For each non-empty document subfolder, delete all files within it using Bash `rm`. Do not delete the folder itself, and do not delete `job-search/documents/README.md`.

```bash
rm -f job-search/documents/cv/*
rm -f job-search/documents/linkedin/*
rm -f job-search/documents/diplomas/*
rm -f job-search/documents/references/*
rm -f job-search/documents/postings/*
rm -rf job-search/documents/applications/*/
```
...
> The skill files are now blank. Run `/setup` to repopulate them. The command auto-detects any files in your `job-search/documents/` folder and offers to read from there; otherwise it walks you through a CV import or interactive interview.
>
> Note that `job-search/CLAUDE.md` and `job-search/cv/main_example.tex` are outside the `profile` scope and still hold your personal data. If you are handing this fork over or making it public, clear them by hand.

**If documents were reset:**
> The `job-search/documents/` folder is now empty. Add your career documents and run `/setup` to populate your profile. See `job-search/documents/README.md` for instructions on what to put where.
