...
- If `$ARGUMENTS` contains `--list`: use Glob with `job-search/.agents/skills/*/SKILL.md`, print a table of installed portal skills (name, market from the description, data source from `url-reference.md`), and stop.
...
2. **Skill name** - kebab-case, suffixed `-search` (e.g. `seek-search`, `stepstone-search`). Must not collide with an existing folder in `job-search/.agents/skills/`.
...
**Canonical reference:** read `job-search/.agents/skills/linkedin-search/` before generating - it is the zero-dependency worked example of this exact structure. Copy its architecture, not its LinkedIn-specific parsing.

Create `job-search/.agents/skills/<name>/` with:
...
- **`job-search/tests/`:** copy `runCLI`/`parseJSON` from `jobindex-search/cli/tests/helpers.ts`, then add a small live smoke-test file: `search` with the test query returns exit code 0 and ≥1 result with non-null `id`/`title`/`url`; a bogus flag or missing required arg exits 1 with a JSON error on stderr.
...
1. Install dev types and typecheck:
   ```bash
   cd job-search/.agents/skills/<name>/cli && bun install && bun run typecheck
   ```
...
1. Ask whether the user wants the new portal added to their `/scrape` search strategy. If yes:
   - The portal CLI itself is already picked up automatically by `/scrape` (it discovers `job-search/.agents/skills/*/SKILL.md`) — no further wiring is needed for CLI search/detail.
...
2. Remind the user to add the install line for their own records if they maintain a fork README:
   ```bash
   cd job-search/.agents/skills/<name>/cli && bun install && cd ../../../..
   ```
...
4. CI coverage is also automatic: the `cli-checks` job discovers every `job-search/.agents/skills/*/cli/package.json`, so the new CLI's `typecheck` and `test` scripts run on every push to the fork without editing the workflow.
...
>
> - Files: `job-search/.agents/skills/<name>/` (SKILL.md, url-reference.md, CLI with tests)
...
>
> Try it: `bun run job-search/.agents/skills/<name>/cli/src/cli.ts search -q "<test query>" --format table`
>
