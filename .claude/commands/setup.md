...
> instead — job-search/SETUP.md section 8 has the two-minute private-remote recipe. Want to
...
Then, before greeting the user, scan the `job-search/documents/` folder. Use Glob with `job-search/documents/**/*` and count files per subfolder (`job-search/cv/`, `linkedin/`, `diplomas/`, `references/`, `applications/`).

Then welcome the user with a single message that lists three paths. The wording changes based on what was found.

**If `job-search/documents/` has files** in one or more subfolders, lead with Path A:
...
>
> I see files in your `job-search/documents/` folder: [list per subfolder, e.g. "2 in job-search/cv/, 1 in linkedin/, 3 in references/"]. Three ways to start:
>
> **Path A: Read my documents folder** (recommended for what you have) - I'll read everything in `job-search/documents/`, cross-reference for consistency, and build your profile from real source materials. Idempotent and safe to re-run as you add more documents.
>
...
**If `job-search/documents/` is empty or missing**, surface Path A as a "do this if you have materials" option:
...
> Three ways to start:
>
> **Path A: Documents folder** (best signal if you have several materials) - Drop your CV / LinkedIn export / diplomas / reference letters in the `job-search/documents/` folder, then say "go". I'll read everything and build your profile from it. See `job-search/documents/README.md` for the folder layout.
>
> **Path B: Single CV import** - Paste or @-mention a single CV/resume here. I'll extract it and ask follow-up questions for what's missing.
>
...
Wait for the user's choice. If they pick A but the folder is still empty, tell them what to add (point at `job-search/documents/README.md`) and stop.

---

## Path A: Documents Folder

Reads structured documents in `job-search/documents/`, cross-references them for consistency, and merges extracted data into the seven profile skill files. Read-before-write and idempotent: changes already present will not be proposed again.

Follow these steps **exactly in order**.

### Step A1: Inventory

Use Glob with `job-search/documents/**/*` to scan the full tree. Print:

```
## Documents Found

**job-search/cv/**: [list files, or "(empty)"]
...
If every subfolder is empty, stop and tell the user to populate the folder. Point at `job-search/documents/README.md` for the layout.
...
Read each document found in Step A1. Process subfolders in this order: `job-search/cv/`, `linkedin/`, `diplomas/`, `references/`, `applications/`.

**`job-search/cv/` documents:** name, contact (email, phone, LinkedIn, GitHub), education (degree, institution, dates, thesis), work experience (title, company, dates, location, bullets), skills, languages (with any stated proficiency), publications, awards, profile/summary.
...
- Languages you work in professionally, with proficiency levels (only if not already extracted from `job-search/cv/` or `linkedin/` above) - this feeds the Language Gate in `04-job-evaluation.md`, so ask directly rather than skipping it
...
Then proceed to Step 3 to populate the non-skill files (`job-search/CLAUDE.md`, `job-search/cv/main_example.tex`, `.claude/skills/job-scraper/search-queries.md`). Step 3 will detect that the seven skill files are already populated and skip those substeps.
...
- **Job portals:** "The framework ships country-agnostic search CLIs (`linkedin-search`, `freehire-search`, enabled by default) plus Danish portal demos (Jobindex, Jobbank, Jobdanmark, Jobnet) that ship **disabled**. `/scrape` auto-discovers whatever portal skills are installed under `job-search/.agents/skills/` and skips any with `enabled: false`. Which portals fit your market?" **Then act on the answer:** if the user's market is Denmark (or they ask for the Danish boards), edit each of the four Danish `SKILL.md` files and set `enabled: true` in the frontmatter; otherwise leave them disabled and say so - they cost nothing while disabled and can be enabled later by flipping the flag. If the user needs a local board that is not shipped, guide them to `/add-portal` (market-specific skills live in their fork). WebSearch/`site:` queries remain the fallback for portals without a CLI skill.
- **CV language:** "Should your CVs be written in English (the default, accepted in most markets), or in your market's language?" Record the answer as a `CV language: <language>` line in job-search/CLAUDE.md's Identity section. Cover letters always match each posting's language automatically; this setting governs the CV only. If the user is unsure, keep English and note they can re-run `/setup --section search` to change it.
...
### 7. Update `job-search/cv/main_example.tex`
...
>
> - `job-search/CLAUDE.md` - Your full candidate profile
...
> - `job-search/cv/main_example.tex` - Your LaTeX CV template
...
> repository instead - see job-search/SETUP.md section 8 for the private-remote setup.
>
> **Try it out:**
...
- Three onboarding paths converge on the same skill files. Step 0 picks the right path based on what's in `job-search/documents/`. Steps 3 and 4 are shared.
