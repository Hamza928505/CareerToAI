...
Use Glob with `job-search/templates/**/TEMPLATE.md` to find registered templates. For each, read the manifest and print a table:
...
2. Use Glob with `job-search/templates/**/TEMPLATE.md` and find manifests whose parent folder name exactly matches `<name>`.
...
7. Derive `<type>` for Step 5 from the manifest path:
   - `job-search/templates/cv/<name>/TEMPLATE.md` -> `cv`
   - `job-search/templates/cover_letters/<name>/TEMPLATE.md` -> `cover_letters`
...
1. **Name** - short kebab-case identifier (e.g. `awesome-cv`, `classic-serif`). Must not collide with an existing folder in `job-search/templates/`.
...
- CV: `job-search/templates/cv/<name>/`
- Cover letter: `job-search/templates/cover_letters/<name>/`
...
2. Compile with the declared compile command, substituting `_compile_test` for `<file>`:
   ```bash
   cd job-search/templates/<type>/<name> && <declared compile command with <file> -> _compile_test>
   ```
...
>
> - **Template skeleton:** `job-search/templates/<type>/<name>/template<source-extension>` — use this as the structural reference instead of the stock template
> - **Manifest:** `job-search/templates/<type>/<name>/TEMPLATE.md` — read this for style rules and known pitfalls before drafting
...
> - **Output file:** `job-search/cv/main_<company>_<role><source-extension>` / `job-search/cover_letters/cover_<company>_<role><source-extension>`; copy any class/package/font files the template needs into the output directory, or reference them by relative path
<!-- END ACTIVE-TEMPLATE -->
```
...
>
> - Files: `job-search/templates/<type>/<name>/` (skeleton, manifest<, class/package files><, fonts>)
