...
1. Read `job-search/job_scraper/notion_sync.json`. Structure:
   ```json
...
5. Write `job-search/job_scraper/notion_sync.json` with the database id and URL. This file is personal state and is gitignored - never commit it.
...
3. **Links** - the posting URL; derive `<company>_<role>` by the **Subfolder naming** rule in `job-search/documents/README.md`, and if that archive exists locally, name its path (plain text - the destination cannot link into the filesystem).
